...
class MainWindow(QtWidgets.QMainWindow):
    ...
    def desmarcarCheckboxColumna2(self):
        row_count = self.mainWindow.tablaTomarAsistencia.rowCount()
        for fila in range(row_count):
            item = self.mainWindow.tablaTomarAsistencia.item(fila, 2)
            if item is not None:
                item.setCheckState(Qt.CheckState.Unchecked)
                # También puedes actualizar la hora correspondiente si es necesario
                hora_item = self.mainWindow.tablaTomarAsistencia.item(fila, 3)
                if hora_item is not None:
                    hora_item.setText("-")
    ...
    def generarReporteAsistencia(self):
        ...
        # Actualizar el estado en la DB
        self.logica_tabla.updateEstadoEstudiante_Unchecked()

        # Limpiar tabla asistencia
